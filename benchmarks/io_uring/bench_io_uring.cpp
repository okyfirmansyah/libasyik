 ...
 *   io_uring avoids per-syscall overhead by batching submissions and completions
 *   through shared kernel/userspace ring buffers, making it the closest thing
 *   to kernel-bypass networking without DPDK.
 *
 ...
 *   GET  /delay/<int>    → busy-wait N ms, {"ok":true}  (no async timer in raw io_uring)
 *
 * Usage:
...
public:
    conn_pool() {
        ...
        for (uint32_t i = MAX_CONNECTIONS; i > 0; --i)
...
static void submit_read(struct io_uring* ring, conn_pool& pool, uint32_t idx) {
    auto& c = pool[idx];
    struct io_uring_sqe* sqe = io_uring_get_sqe(ring);
    io_uring_prep_recv(sqe, c.fd, c.buf + c.buf_used,
                       BUF_SIZE - c.buf_used, 0);
...
static void submit_write(struct io_uring* ring, conn_pool& pool, uint32_t idx,
    ...
    c.write_buf = data;
    struct io_uring_sqe* sqe = io_uring_get_sqe(ring);
    io_uring_prep_send(sqe, c.fd, c.write_buf.data(),
                       c.write_buf.size(), 0);
...
static void submit_timeout(struct io_uring* ring, conn_pool& pool, uint32_t idx,
    ...
    // Store the timespec in the body_buf memory (we need it to live until completion)
