...
class digest_authenticator {
 public:
  digest_authenticator(string_view www_authenticate, string_view username,
                       string_view password, string_view uri,
                       string_view method, string_view responseBody) noexcept
  ...
  bool generateAuthorization()
  {
    ...
    m_authorization.reserve(128 + m_username.size() + m_realm.size() +
                            m_nonce.size() + m_uri.size() +
                            m_nonceCount.size() + m_cnonce.size() +
 ...
 private:
  enum QualityOfProtection { None, Auth, AuthInt };
  ...
  bool findSection(const std::string& key, string_view& value) const
  {
    boost::regex reg{key + "=([^,]+)"};
    auto start = m_authenticate.cbegin();
    auto end = m_authenticate.cend();
    boost::match_results<string_view::const_iterator> matches;
  ...
  string_view m_authenticate;
  string_view m_username;
  string_view m_password;
  string_view m_realm;
  string_view m_nonce;
  string_view m_opaque;
  string_view m_algorithm;
  string_view m_uri;
  string_view m_method;
  string_view m_body;
